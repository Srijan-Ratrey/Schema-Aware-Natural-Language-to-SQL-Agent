...
# Shared train/inference serialization (single source of truth).
try:
    from .schema_serialization import build_input as _build_input
except ImportError:  # allow running this module standalone
    from schema_serialization import build_input as _build_input
...
class NL2SQLModel:
    ...
    def _prepare_input(self, query: str, schema: str) -> str:
        """
        Build the model input in the shared train/inference format.

        ``schema`` is expected to be a serialized schema string produced by
        ``src.schema_serialization.serialize_schema`` (i.e. the real, full schema
        of whatever DB is connected). Keeping this delegated to the shared helper
        is what makes the model schema-aware for *any* database rather than the
        bundled demo schema.
        """
        schema_str = (schema or "").strip()
        # build_input prepends "translate to SQL:" and a "schema:" section.
        # We pass the already-serialized schema through unchanged.
        return f"translate to SQL: {query} | schema: {schema_str}"

    def _clean_sql(self, sql: str) -> str:
        """
        Minimal, schema-agnostic cleanup of generated SQL.

        Deliberately does NOT rewrite table/column names or guess fallbacks — that
        logic used to be hardcoded to the demo schema and corrupted SQL on any other
        database. Identifier reconciliation now happens (schema-driven) in
        NL2SQLAgent._correct_table_names.
        """
        if not sql:
            return ""
        ...
        # Drop an echoed prompt prefix or a leading label (SQL:, Answer:, Query:).
        sql = re.sub(r'^.*?translate to sql:\s*', '', sql, flags=re.IGNORECASE)
        sql = re.sub(r'^(SQL|Answer|Query)\s*:\s*', '', sql, flags=re.IGNORECASE)

        # Collapse whitespace.
        sql = ' '.join(sql.replace('\n', ' ').replace('\t', ' ').split())

        # Keep only the first statement (defensive against trailing text).
        if ';' in sql:
            sql = sql.split(';', 1)[0].strip()

        # Must look like an actual SQL statement.
        if not any(kw in sql.upper() for kw in ('SELECT', 'INSERT', 'UPDATE', 'DELETE', 'WITH')):
            return ""
